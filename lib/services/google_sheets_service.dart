...
class GoogleSheetsService {
  ...
  Future<List<Map<String, dynamic>>> getPayments() async {
    try {
      final url = 'https://docs.google.com/spreadsheets/d/$sheetId/gviz/tq?tqx=out:csv&sheet=$sheetName';
  ...
  List<Map<String, dynamic>> _parseCsv(String csvData) {
    ...
    try {
      ...
      if (latestPayment != null) {
        final description = latestPayment['Mô tả']?.toString().toUpperCase() ?? '';
        final amountStr = latestPayment['Giá trị']?.toString().replaceAll(',', '').replaceAll('.', '') ?? '0';
        ...
        final isContentMatch = normalizedDescription.contains(normalizedTransferContent);
        final isAmountMatch = amount >= expectedAmount * 0.99 && amount <= expectedAmount * 1.01; 
      ...
      for (var payment in allPayments) {
        ...
        final amountStr = payment['Giá trị']?.toString().replaceAll(',', '').replaceAll('.', '') ?? '0';
        final amount = double.tryParse(amountStr) ?? 0;
